    ...
    return {
      send: (options) => {
        return new Promise((resolve, reject) => {
          ...
          if (!_.isNil(options.cc)) {
            msg.cc = options.cc
          }
          if (!_.isNil(options.bcc)) {
            msg.bcc = options.bcc
          }
          if (
            !_.isNil(options.attachments) &&
            _.isArray(options.attachments) &&
            !_.isEmpty(options.attachments)
          ) {
            msg.attachments = options.attachments
          }

          strapi.log.debug('Sending email message', msg)
